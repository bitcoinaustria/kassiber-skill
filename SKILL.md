...
35. If `kassiber status` (or any other command) returns `passphrase_required`, the local DB is SQLCipher-encrypted. A human can enroll prompt-free one-shot CLI commands once with `kassiber secrets remember-unlock`; this creates a CLI-only OS credential that is separate from desktop Touch ID enrollment. Otherwise prompt interactively or have the controlling process use `--db-passphrase-fd <FD>`. Never ask the agent to see, retain, or place the passphrase in argv — there is no `--db-passphrase <value>` flag.
