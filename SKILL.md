...
- `admin` is never a standing broker capability. A brokered admin command requires a fresh passphrase through the global `--operator-auth-fd FD`; never supply it in argv, JSON, or chat.
